...
class ScheduleDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        ttk.Combobox(r, textvariable=self._loop_var, values=self._loop_names,
        ...
        ttk.Combobox(r2, textvariable=self._sc_var, values=["daily", "once"],
        ...
        tk.Entry(r3, textvariable=self._st_var, width=10).pack(side="left", padx=4)
        ...
        tk.Entry(r3, textvariable=self._sd_var, width=12).pack(side="left", padx=4)

        tk.Button(self, text="+ สร้าง Schedule", bg="#4ec9b0",
                  command=self._create).pack(pady=6)
        ...
        tk.Button(b, text="ลบ task ที่เลือก", fg="red", command=self._delete).pack(side="left")
        tk.Button(b, text="ปิด", command=self.destroy).pack(side="right")
